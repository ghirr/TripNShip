...
public class UpdateVehicleController {
    ...
    @FXML private ComboBox<RentalAgency> agencyComboBox;
    ...
    public void initialize() {
        ...
        if (vehicleToUpdate != null) {
            ...
            agencyComboBox.setValue(vehicleToUpdate.getAgency());
        }

        // Configure le bouton de mise à jour
        updateVehicleButton.setOnAction(event -> handleUpdateVehicle());

        // Charger les agences dans le ComboBox
        agencyComboBox.getItems().addAll((RentalAgency) vehicleService.getAll());
    }
    ...
    private void handleUpdateVehicle() {
        ...
        RentalAgency agency = agencyComboBox.getValue();

        // Créer un objet Vehicle avec les nouvelles données
        Vehicle updatedVehicle = new Vehicle(vehicleToUpdate.getIdVehicle(), brand, model, licensePlate, dailyPrice, availability, type, agency);
